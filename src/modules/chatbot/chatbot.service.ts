...
export class ChatbotService {
  ...
  private async buildContext(role: string, userId?: string): Promise<string> {
    ...
    if (role === 'admin') {
      ...
      employes.forEach(e => parts.push(`- ${e.prenom || '?'} ${e.nom || '?'} (${e.email}), téléphone: ${e.telephone || 'N/A'}, poste: ${e.poste || 'N/A'}, département: ${e.departement || 'N/A'}, entreprise: ${e.entrepriseText || 'N/A'}`));
  ...
  private async callGroq(question: string, role: string, context: string): Promise<string> {
    ...
    const systemPrompt = `Tu es un assistant chatbot pour la plateforme StirForma (gestion de formations). 
...
Utilise des émojis avec parcimonie.`;
  ...
  private async extractParams(question: string, forcedAction: string): Promise<ActionIntent | { type: 'question' } | null> {
    ...
    const paramHints: Record<string, string> = {
      ...
      add_employe: 'Extrais les informations de l\'employé: nom (obligatoire), prenom (obligatoire), email (obligatoire), telephone, poste, departement, entrepriseText, identifiant.',
      add_participant: 'Extrais les informations du participant: nom (obligatoire), prenom (obligatoire), email (obligatoire), telephone, poste, departement, entrepriseText.',
...
- add_employe: Ajouter un employé (nom, prenom, email obligatoires, optionnels: telephone, poste, departement, entrepriseText, identifiant)
- add_participant: Ajouter un participant (nom, prenom, email obligatoires, optionnels: telephone, poste, departement, entrepriseText)
...
Si action: {"type": "action", "action": "nom_action", "parameters": {"champ": "valeur"}}`;
  ...
  private async addEmploye(params: Record<string, any>): Promise<string> {
    ...
    const saved = await this.employeService.create({
      ...
      entrepriseText: params.entrepriseText || undefined,
    });
  ...
  private async addParticipant(params: Record<string, any>): Promise<string> {
    ...
    const defaultPassword = 'StirForma2024!';
    ...
    const user = this.userRepo.create({
      ...
      entrepriseText: params.entrepriseText || null,
      isActive: true,
    });
    ...
    await this.mailService.send({
      to: saved.email,
      subject: 'Bienvenue sur StirForma — Vos identifiants',
      html: `<p>Bonjour ${saved.prenom} ${saved.nom},</p><p>Votre compte participant a été créé sur StirForma.</p><p><b>Email :</b> ${saved.email}<br><b>Mot de passe :</b> ${defaultPassword}</p><p>Connectez-vous pour accéder à vos formations.</p>`,
    });
  ...
  private async addFormateur(params: Record<string, any>): Promise<string> {
    ...
    await this.mailService.send({
      to: saved.email,
      subject: 'Bienvenue sur StirForma — Vos identifiants formateur',
      html: `<p>Bonjour ${saved.prenom} ${saved.nom},</p><p>Votre compte formateur a été créé sur StirForma.</p><p><b>Email :</b> ${saved.email}<br><b>Mot de passe :</b> ${defaultPassword}</p><p>Connectez-vous pour gérer vos sessions.</p>`,
    });
  ...
  private async deactivateUser(params: Record<string, any>): Promise<string> {
    ...
    await this.mailService.send({
      to: user.email,
      subject: 'Votre compte StirForma a été désactivé',
      html: `<p>Bonjour ${user.prenom} ${user.nom},</p><p>Votre compte StirForma a été désactivé par l'administration.</p><p>Pour plus d'informations, contactez le support.</p>`,
    });
