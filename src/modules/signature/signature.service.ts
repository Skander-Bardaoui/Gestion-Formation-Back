...
import { CreateDocumentSigneDto } from './dto/create-document-signe.dto';
...
export class SignatureService {
  ...
  private async generateConventionPdf(options: {
    ...
    doc.text('STIRFORMA - Centre de formation professionnelle');
    ...
    if (participant.entrepriseText) doc.text(`Entreprise : ${participant.entrepriseText}`);
    doc.moveDown(0.5);
  ...
  private async generateEmargementPdf(options: {
    ...
    if (participants.length === 0) {
      ...
      participants.forEach((p, idx) => {
        const lineY = rowY;
        const hasSig = participantSignatures.has(p.id);
